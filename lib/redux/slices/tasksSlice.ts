...
export const createTask = createAsyncThunk<TaskThunkResult, CreateTaskPayload, { rejectValue: TaskThunkError }>(
  'tasks/createTask',
  async (task: CreateTaskPayload, { rejectWithValue }) => {
    try {
      ...
      const { payload, message } = await readResponsePayload(res, 'Failed to create task');
...
export const updateTask = createAsyncThunk<TaskThunkResult, { id: string; data: Partial<Task> }, { rejectValue: TaskThunkError }>(
  'tasks/updateTask',
  async ({ id, data }: { id: string; data: Partial<Task> }, { rejectWithValue }) => {
    try {
      ...
      const { payload, message } = await readResponsePayload(res, 'Failed to update task');
      const taskPayload = payload && typeof payload === 'object' && 'task' in payload && (payload as { task?: Task }).task
        ? (payload as { task: Task }).task
        : (payload as Task);
      return { task: taskPayload, message: message || undefined };
...
export const updateTaskProgress = createAsyncThunk<TaskThunkResult, { id: string; startDate?: string; endDate?: string; status?: TaskStatus }, { rejectValue: TaskThunkError }>(
  'tasks/updateTaskProgress',
  async ({ id, startDate, endDate, status }: { id: string; startDate?: string; endDate?: string; status?: TaskStatus }, { rejectWithValue }) => {
    try {
      ...
      const { payload, message } = await readResponsePayload(res, 'Failed to update progress');
      const taskPayload = payload && typeof payload === 'object' && 'task' in payload && (payload as { task?: Task }).task
        ? (payload as { task: Task }).task
        : (payload as Task);
      return { task: taskPayload, message: message || undefined };
...
export const logHours = createAsyncThunk<TaskThunkResult, { id: string; data: { date: string; hours: number; notes?: string; employee?: string; employeeId?: string; startTime?: string; endTime?: string } }, { rejectValue: TaskThunkError }>(
  'tasks/logHours',
  async ({ id, data }: { id: string; data: { date: string; hours: number; notes?: string; employee?: string; employeeId?: string; startTime?: string; endTime?: string } }, { rejectWithValue }) => {
    try {
      ...
      const { payload, message } = await readResponsePayload(res, 'Failed to log hours');
      const taskPayload = payload && typeof payload === 'object' && 'task' in payload && (payload as { task?: Task }).task
        ? (payload as { task: Task }).task
        : (payload as Task);
      return { task: taskPayload, message: message || undefined };
...
export const addPhoto = createAsyncThunk<TaskThunkResult, { id: string; photoUrl: string; employee?: string; employeeId?: string }, { rejectValue: TaskThunkError }>(
  'tasks/addPhoto',
  async ({ id, photoUrl, employee, employeeId }: { id: string; photoUrl: string; employee?: string; employeeId?: string }, { rejectWithValue }) => {
    try {
      ...
      const { payload, message } = await readResponsePayload(res, 'Failed to add photo');
      const taskPayload = payload && typeof payload === 'object' && 'task' in payload && (payload as { task?: Task }).task
        ? (payload as { task: Task }).task
        : (payload as Task);
      return { task: taskPayload, message: message || undefined };
